/*
    ...
    Fullscreen-optimized fork for desktop wallpaper use:
    https://www.shadertoy.com/view/fcf3Dn

    --- neowall perf pass ---
    Changes from upstream, for use as a 3-monitor desktop wallpaper on an
    RX 6700 XT where frame time was pinned right at the 16.7ms/60fps budget:

      - AA: 2 -> 1            (was running the entire render() 4x/pixel;
                                this is the ~4x win, basically invisible on
                                a slow ambient wave viewed at desktop distance)
      - MAX_DRAWS: 40 -> 16    (fewer fine glow-shell substeps; alpha is
                                clamped downstream anyway, so visual delta
                                is small)

    Original values left in comments below if you want to A/B by eye.
*/
...
{ 
    ...
    for (int i = 0; i < 64; i++)   // was 100 — most rays resolve well before this
    {
        vec3 p = o + d * t;
